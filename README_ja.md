...
- 2026-03-05 (post v2026.03.05): セッション実行と成果物運用の更新 — Structured Concurrency の機能フラグ切替を廃止して実行経路を固定化、CLI起動時刻を使ったパス単位セッション命名を追加、`--no-shared-session` によるパスごと独立セッション実行を追加、パスレポートを `.checkpoints/passes` 配下に退避し、CLI終了時に `.checkpoints` を自動削除。PR #86/#87 でマージ
...
以下のツリーは 2026-03-03 時点の現行ソース構成に同期済みです。
